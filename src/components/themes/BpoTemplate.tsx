...
import { Phone, MessageCircle, CheckCircle, ShieldCheck, Clock, Headphones, ChevronDown, ChevronUp, MapPin, Mail, Users } from 'lucide-react';
...
export default function BpoTemplate({ businessName, phone, suburb }: TemplateProps) {
      ...
      .insert([
        {
          ...
          phone: leadPhone,
          message: `[BPO Inquiry - ${suburb}] ${leadMessage}`
        }
      ]);
    ...
    <div className="bg-slate-950 text-slate-100 min-h-screen font-sans selection:bg-blue-600 selection:text-white">
      ...
      <header className="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-md border-b border-slate-800/80 px-8 py-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center gap-8 text-xs font-bold text-slate-300 uppercase tracking-wider">
          ...
          <a href="#projects" className="hover:text-blue-400 transition">Our Work</a>
      ...
      <section className="relative px-8 py-24 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div className="flex flex-col gap-6">
          <div className="inline-flex items-center gap-2 bg-blue-600/10 border border-blue-500/20 px-3.5 py-1.5 rounded-full text-blue-400 text-xs font-bold w-max">
            <ShieldCheck className="w-4 h-4" /> INTERNATIONAL CALL CENTER & BPO SOLUTIONS
          </div>
          <h1 className="text-4xl sm:text-6xl font-black tracking-tight text-white leading-[1.1]">
            PROFESSIONAL CALL CENTER AND BPO SERVICES IN <span className="text-blue-500">{suburb.toUpperCase()}</span>
          </h1>
          <p className="text-slate-300 text-base max-w-lg">
            Support your business operations with our reliable telecalling, appointment setting, and back-office solutions tailored for your local or global needs from {suburb}.
          </p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-6 border-t border-slate-900 text-xs font-bold text-slate-400">
            <div className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-emerald-400" /> Tele Calling</div>
            <div className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-emerald-400" /> Appointment Setting</div>
            <div className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-emerald-400" /> Accurate Back Office</div>
            <div className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-emerald-400" /> Multichannel</div>
          </div>
        </div>

        <div className="relative">
          <div className="absolute -inset-1 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-3xl blur-xl opacity-30 animate-pulse"></div>
          <div className="relative bg-slate-900 border border-slate-800 p-8 rounded-3xl flex flex-col gap-6 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <span className="font-bold text-sm text-white">Operations Infrastructure</span>
              <span className="w-2.5 h-2.5 rounded-full bg-blue-500 animate-ping"></span>
            </div>
            <div className="space-y-3">
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/80 text-xs text-slate-300">
                🎧 24/7 Omnichannel support infrastructure supporting voice, email, and live chat campaigns.
              </div>
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/80 text-xs text-slate-300">
                📊 Dedicated account managers and rigorous quality assurance monitoring.
              </div>
            </div>
            <a 
      ...
      <section id="services" className="py-20 px-8 max-w-7xl mx-auto border-t border-slate-900">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="text-slate-400 text-sm mt-2">We streamline your customer outreach and administrative tasks with dedicated telecalling, precise appointment setting, and efficient back office support.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { title: 'Tele Calling', desc: 'Outbound and inbound calling solutions to connect with your customers, conduct surveys, and handle inquiries efficiently.' },
            { title: 'Appointment Setting', desc: 'Professional scheduling services to secure qualified meetings and sales opportunities for your internal business teams.' },
            { title: 'Back Office Support', desc: 'Reliable data entry, document processing, and administrative support to keep your daily business operations running smoothly.' }
          ].map((service, i) => (
      ...
      <section id="why-us" className="py-20 px-8 max-w-7xl mx-auto border-t border-slate-900">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <h3 className="text-3xl font-black text-white tracking-tight">Why Choose Us</h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { title: 'Professional Tele Calling', desc: 'Connect directly with your target audience through experienced call center agents in ' + suburb + '.' },
            { title: 'Streamlined Appointment Setting', desc: 'Fill your calendar with qualified meetings managed by our dedicated outbound calling team.' },
            { title: 'Accurate Back Office', desc: 'Free up internal resources by outsourcing repetitive administrative tasks and data management.' },
            { title: 'Multichannel Communication', desc: 'Reach us easily or manage your campaigns through phone, email, and WhatsApp channels.' }
          ].map((item, i) => (
      ...
      <section id="projects" className="py-20 px-8 max-w-7xl mx-auto border-t border-slate-900">
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <h3 className="text-3xl font-black text-white tracking-tight">Our Work</h3>
        </div>
          ...
          {[1, 2, 3].map((p) => (
            <div key={p} className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden flex flex-col group">
              <div className="h-48 bg-slate-950 flex items-center justify-center text-slate-700 font-bold text-sm group-hover:scale-105 transition duration-500">
                Campaign Case Study #{p}
              </div>
              <div className="p-6 flex flex-col gap-2">
                <span className="text-[10px] font-bold text-blue-400 uppercase tracking-wider">Enterprise BPO Campaign</span>
                <h4 className="font-bold text-white text-base">Outbound Lead Generation & CRM Sync</h4>
                <p className="text-xs text-slate-400">Scaled inbound conversion rates by 40% with automated follow-up workflows.</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="areas" className="py-20 px-8 max-w-7xl mx-auto border-t border-slate-900">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <h3 className="text-3xl font-black text-white tracking-tight">Global & Local Reach</h3>
        </div>
        <div className="flex flex-wrap justify-center gap-3">
          {[suburb, 'Andheri', 'Bandra', 'Thane', 'Navi Mumbai', 'Borivali', 'Global Clients'].map((area, i) => (
      ...
      <section id="faq" className="py-20 px-8 max-w-4xl mx-auto border-t border-slate-900">
        ...
        <div className="flex flex-col gap-4">
          {[
            { q: `What services does ${businessName} offer?`, a: 'We specialize in providing telecalling, appointment setting, and comprehensive back-office outsourcing services.' },
            { q: 'How can we contact your Mumbai call center?', a: `You can reach our ${suburb} office via phone, email, or WhatsApp anytime.` },
            { q: 'Do you handle both inbound and outbound campaigns?', a: 'Yes, our agents are trained for full-scale omnichannel inbound customer service and outbound lead gen.' }
          ].map((faq, i) => (
      ...
      <footer id="contact" className="py-20 px-8 max-w-7xl mx-auto border-t border-slate-900 grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="flex flex-col gap-6">
          <div>
            ...
            <h3 className="text-3xl font-black text-white tracking-tight">Get Started with {businessName}</h3>
            <p className="text-slate-400 text-sm mt-2">Contact us today via phone, email, or WhatsApp to discuss your call center and back office needs.</p>
          </div>
            ...
            <form onSubmit={handleLeadSubmit} className="flex flex-col gap-4">
              ...
              <div>
                ...
                <textarea 
                  ...
                  placeholder="Describe your BPO requirements..." 
                  className="mt-1 w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white focus:outline-none focus:border-blue-500" 
                />
              </div>
              <button 
